...
router.get('/main-stats',  (req,res,next)=>next(), async (req, res) => {
    try {
...
router.get('/admin-stats',  (req,res,next)=>next(), async (req, res) => {
    try {
        ...
        // Grouping logic for more detail
        ...
        activeStudentsByCourse.forEach(item => {
            studentsByCourse[item.courseInterest || 'Geral'] = parseInt(item.get('count'));
        });

        const classesByCourse = {};
        activeClassesByCourse.forEach(item => {
            classesByCourse[item.Course?.name || 'Geral'] = parseInt(item.get('count'));
        });
...
router.get('/admin-charts',  (req,res,next)=>next(), async (req, res) => {
    try {
        ...
        for (const s of students) {
            // Student has courseInterest or model relationship if complex, using simple field here
            const c = s.courseInterest || 'Geral';
            courseCounts[c] = (courseCounts[c] || 0) + 1;
        }
        const courseData = Object.keys(courseCounts).map(name => ({ name, value: courseCounts[name] }));
...
router.get('/financial-stats',  (req,res,next)=>next(), async (req, res) => {
    try {
...
router.get('/my-stats',  (req,res,next)=>next(), async (req, res) => {
    try {
        const requester = req.user;
