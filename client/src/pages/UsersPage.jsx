...
const UsersPage = () => {
    ...
    const handleRegisterSave = async (newUser) => {
        try {
            // Remove ID generated by RegisterUser to let backend handle it, unless backend expects it
            // Backend usually generates. User provided code had mock ID.
            // But api.createUser takes what it gets.
            // Let's ensure strict fields.
            const { id, ...payload } = newUser;
            const created = await api.createUser(payload);
            setSuccessData({ ...created, password: newUser.password });
            loadData();
            setShowModal(false);
        } catch (error) {
